...
public class Leistungscodes extends PreferencePage implements IWorkbenchPreferencePage {
				...
				if (idx != -1) {
					...
					for (String s1 : systeme) {
						if (s1.equals(ssel)) {
							...
							pre[7] = BillingSystem.getConfigurationValue(s1, BillingSystem.CFG_BILLINGLAW, BillingLaw.KVG.name());
							pre[8] = BillingSystem.getConfigurationValue(s1, BillingSystem.CFG_NOCOSTBEARER, Boolean.FALSE.toString());
	...
	class AbrechnungsTypDialog extends TitleAreaDialog {
		...
		protected Control createDialogArea(final Composite parent){
			...
			bNoCostBearer.setText("Kostenträger-Auswahl ausblenden (zB Privat-Fall)");
